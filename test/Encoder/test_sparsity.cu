...
auto para = std::make_shared<Model_t>(Model_t{768, 768, 256, 12, 768 * 4, 768});
...
void test() {
    ...
    using attn_config =
        Attention_config<w_mat_t, w_mat_t, mat_v_t, w_mat_t, attn_t>;
    ...
    for (int i = 0; i < 20; i++) {
        ...
        for (int i = 0; i < model.size(); i++) {
            ...
            auto attn = std::make_unique<Attention<attn_config>>(
                std::move(LQ), std::move(LK), std::move(LV), std::move(LO),
                para);
